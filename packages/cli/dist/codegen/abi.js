...
module.exports = class AbiCodeGenerator {
    ...
    _generateCallTypes() {
        let callFunctions = util.disambiguateNames({
            ...
            getName: fn => fn.get('name') || (fn.get('type') === 'constructor' ? 'constructor' : 'default'),
        ...
        return callFunctions
            ...
            util
                ...
                .forEach((input, index) => {
                let callInput = this._generateInputOrOutput(input, index, fnClassName, `call`, `inputValues`);
            ...
            util
                ...
                let callInput = this._generateInputOrOutput(output, index, fnClassName, `call`, `outputValues`);
    ...
    _generateEventTypes() {
        ...
        return events
            ...
            inputs.forEach((input, index) => {
                ...
                let paramObject = this._generateInputOrOutput(input, index, eventClassName, `event`, `parameters`);
    ...
    _generateInputOrOutput(inputOrOutput, index, parentClass, parentType, parentField) {
        ...
        let valueType = parentType === 'event' && inputOrOutput.get('indexed')
    ...
    _generateTupleType(inputOrOutput, index, parentClass, parentType, parentField) {
        ...
        let tupleGetter = tsCodegen.method(`get ${name}`, [], util.isTupleArrayType(type) ? `Array<${tupleClassName}>` : tupleClassName, `
      return ${typesCodegen.ethereumToAsc(parentType === 'tuple'
            ? `this[${index}]`
            : `this._${parentType}.${parentField}[${index}].value`, type, tupleClassName)}${util.isTupleType(type) ? ` as ${tupleClassName}` : ''}
      `);
        ...
        inputOrOutput.get('components').forEach((component, index) => {
            ...
            let paramObject = this._generateInputOrOutput(component, index, tupleIdentifier, `tuple`);
    ...
    _generateSmartContractClass() {
        ...
        klass.addMethod(tsCodegen.staticMethod('bind', immutable.List([
        ...
        functions.forEach(member => {
            ...
            `));
                ...
                if (util.containsTupleType(type)) {
                    ...
                    let tuple = this._generateTupleType(outputs.get(0), 0, tupleResultParentType, 'function', this.abi.name);
            ...
            inputs.forEach((input, index) => {
                if (util.containsTupleType(input.get('type'))) {
                    types = types.concat(this._generateTupleType(input, index, tupleInputParentType, 'function', this.abi.name).classes);
                }
            });
            // Generate and add a method that implements calling the function on
            // the smart contract
            let params = inputs.map((input, index) => tsCodegen.param(input.get('name'), this._getTupleParamType(input, index, tupleInputParentType)));
            let superInputs = `
      '${fnName}',
      '${fnSignature}',
      [${inputs.size > 0
      ...
      ${simpleReturnType
                ...
                : `new ${returnType.name}(
                ${outputs
                    .map((output, index) => `${typesCodegen.ethereumToAsc(isTry ? `value[${index}]` : `result[${index}]`, output.get('type'), util.isTupleArrayType(output.get('type'))
